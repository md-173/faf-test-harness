...
| `--lobby-port <port>` | yes | none | A fallback UDP port for peer traffic. The game binds the port the adapter announces in `CreateLobby`, logging a warning if the two differ, and binds this one only when that frame carries no usable port. |
