...
(`--gpgnet-port`, `--lobby-port`, `--player-id`, `--player-login`). Having no
lobby, this diagnostic takes the player identity from `playerIdOverride` and
`playerLogin` rather than from the lobby `welcome` a full `run` uses.
