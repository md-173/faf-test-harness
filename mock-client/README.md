...
| `70` | `RUNTIME`         | A runtime failure after a subcommand started, e.g. `run` had no usable refresh-token file, the lobby session failed, its ICE adapter or game never came up (a binary that could not be started, an adapter that exited or never accepted its JSON-RPC connection, one that refused a setup call, or an unexpected exception on the way up; logged as `the ICE adapter or game never came up`, after a line naming the cause), or its session failed after they came up (a `HostGame`, `JoinGame` or `ConnectToPeer` frame it could not read, an adapter that answered one of those calls with an error or not within 5 s, a match the server cancelled after `game_launch`, or an unexpected exception in the host or join step; logged as `the session failed after its ICE adapter and game came up`, after a line naming the cause), `launch-ice` / `launch-game` could not find/start its binary, the child exited before its run window, `launch-ice` could not attach a JSON-RPC peer to the adapter it started (WBS-3.1.6.3), `ice-smoke` returned any verdict other than reachable, or a `session` checkpoint failed (including no two-way game traffic) or a subprocess survived its teardown. Also any exception that escapes a subcommand uncaught. |
...
adapter's connection closed is not a failed session at all: the adapter is gone,
which is `72`'s finding, though until #438 lands a run that such a failure ends
exits `0` (see the `72` row). A run that a signal ended logs none of these lines,
because its exit code is the signal's own, unless the session's own end and the
signal land in the same instant.
