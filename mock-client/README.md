...
| `69` | `ADAPTER_LOST`    | `run` only: the session ran, but its game reported that the GPGNet link to the adapter went down, exiting with mock-game's own `ADAPTER_LOST`. Same number as mock-game's code because it is the same event seen from the other side. Decided from the code the game reported rather than from whether teardown had run, which races this classification. An adapter that exits without the game noticing is still only a warning. |
