...
| `69` | `ADAPTER_LOST`    | `run` only: the session ran, but its game reported that the GPGNet link to the adapter went down, exiting with mock-game's own `ADAPTER_LOST`, and no shutdown signal explains it. Same number as mock-game's code because it is the same event seen from the other side. Deliberately not conditioned on teardown having run, which races this classification. An adapter that exits without the game noticing is still only a warning. |
...
| `71` | `GAME_CRASHED`    | `run` only: the session ran, but the game process died unaccounted for: a non-zero exit with no `GameEnded` frame observed and no harness-initiated teardown, the same condition that logs `mock-game exited abnormally`. Covers a game that failed to start as well as one that died mid-match, but not a reported adapter loss, which is `69`. Before this existed, such a run exited `0`. |
