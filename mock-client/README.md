...
exits `0` (see the `72` row). A live adapter whose RPC stream stopped parsing
fails its calls the same way, and until #452 lands that run exits `0` too. A run
that a signal ended logs none of these lines, because its exit code is the
signal's own, unless the session's own end and the signal land in the same
instant.
...
³ `--unique-id` and `--uid-binary-path` are mutually exclusive. A live lobby connection requires a generated uid that can only be created by the `faf-uid` program so in that case only `--uid-binary-path` should be set. `--unique-id` is used when not communicating with the live lobby server (e.g. a scripted server taking its place)
