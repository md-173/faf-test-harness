...
final class CliTestFixtures {
     ...
     */
    static String[] withSubcommandAndIceBinary(
            final String subcommand, final String iceBinaryPath) {
        return Stream.concat(Stream.of(subcommand), Stream.of(minimalRequiredFlags()))
                .map(
                        arg ->
    ...
    /**
     * Like {@link #withSubcommand(String)} but replaces {@code --mock-game-binary-path} with {@code
     * mockGameBinaryPath}. Tests that exercise {@code launch-game}'s "binary not found" path pass a
     * guaranteed-absent path under a JUnit {@code @TempDir} so the assertion does not depend on the
     * Gradle install layout being present (it is generated by {@code installDist}, which {@code
     * ./gradlew build} runs ahead of {@code test}).
     */
    static String[] withSubcommandAndGameBinary(
            final String subcommand, final String mockGameBinaryPath) {
        return Stream.concat(Stream.of(subcommand), Stream.of(minimalRequiredFlags()))
                .map(
                        arg ->
                                arg.startsWith("--mock-game-binary-path=")
