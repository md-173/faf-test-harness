...
public final class SessionPeer {
    ...
    /**
     * Records one {@code onConnected} notification off the R36 fan-out. Malformed ones are dropped
     * rather than failing here: this runs on the adapter's reader thread, where an exception would
     * be swallowed, so a missing verdict surfaces as the checkpoint that timed out instead.
     *
    ...
    private void recordVerdict(final JsonNode notification) {
        ...
        if (!params.isArray() || params.size() < VERDICT_PARAMS || !params.get(2).isBoolean()) {
            return;
        }
        verdicts.add(
                new PeerVerdict(
