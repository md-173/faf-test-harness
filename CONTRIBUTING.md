...
- **Credentials are per-peer and CI-only.** CI's accounts are never used by a local live run, because a second login as the same account signs the first out, fatally; the `concurrency` group is keyed on those accounts rather than on the branch for the same reason.
  - **Today** the job reads `FAF_CI_REFRESH_TOKEN_C` and `FAF_CI_REFRESH_TOKEN_D`, which must hold *refresh* tokens, because the workflow still passes `--peer-refresh-token-file`. `session` itself accepts `--peer-access-token-file` since #87's access-token step; switching the workflow is the #364 follow-up. Hydra rotates a refresh token on every use and the rotated value dies with the runner, so each run spends them: delete both secrets straight after a run and re-bootstrap those accounts per [`harness-runbook.md`](documentation/operations/harness-runbook.md) §3.
  - **Once the workflow switches** (the #364 follow-up), the job uses one pre-signed access token per peer, and `scripts/ci/mint-access-token.sh <refresh-token-file> <secret-name>` becomes the way to set them right before a dispatch; its `--dry-run` prints the token's subject, scopes and expiry without setting anything. An access token lasts about an hour, so a dispatch that waits in the queue may need a fresh one.
