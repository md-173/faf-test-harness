...
**Mock Game policy:** treat the GPGNet connection as usable only once the Mock Client's JSON-RPC connection to the adapter is established. This is an ordering constraint *across* components, which [3.2.4.1] and [3.1.2.7] must honour. The Mock Client already satisfies it because it connects its adapter transport before the game is launched. A standalone game-side test has no client to rely on and must stand in for one by holding a TCP socket open on the adapter's RPC port; `GpgNetConnectionLiveSmokeTest` (3.2.2.4) does that and additionally waits, before its first frame, for two adapter log lines that together prove the `currentClient` assignment above has completed (WBS 3.1.2.10 / #225).
