...
| A session that failed after its adapter and game came up | `70` (`RUNTIME`) | A `HostGame`, `JoinGame` or `ConnectToPeer` frame the client could not read, an adapter that answered one of those calls with an error or not within 5 s, or, when matchmaking, a match the server cancelled after `game_launch` and before the game started (once `Match started` is logged, the match plays on, #304). The cause is logged first (`Could not host the game`, `Could not join the game`, `Could not connect to a peer`, `Could not set up the peer relay for id=<id>` or `match_cancelled after game_launch`), then `the session failed after its ICE adapter and game came up; reporting it in this run's exit code`. An unexpected exception in the host, join or peer-connect step is logged like the launch's, at ERROR with its trace. A call that failed because the adapter's connection closed is not this: that adapter is gone, which is the `72` row's to report, so the line naming the call is INFO. Until #438 lands, a run that such a failure ends still exits `0`, as the `72` row says. A live adapter whose RPC stream stopped parsing fails its calls the same way, and until #452 lands that run exits `0` too. Checked after `72` and `71`, so a run whose adapter or game died reports that instead. |
