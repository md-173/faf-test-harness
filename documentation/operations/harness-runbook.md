...
| Runtime failure after start (bad refresh-token file, lobby session failure), or a game launch whose ICE adapter or game never came up | `70` (`RUNTIME`) | A failed launch logs its cause first (`Could not launch the ICE adapter`, `Could not connect or setup the ICE adapter` or `Could not launch game binary`), then `the ICE adapter or game never came up; reporting it in this run's exit code`. `run` does not check the binaries before it logs in, so a wrong path only surfaces once the lobby sends `game_launch`. An unexpected exception on the way up is a bug, not a finding about the adapter: its cause line is `Could not launch the game session` at ERROR with the stack trace, and the run ends the same way instead of waiting forever. Like the real client, `run` still sends the lobby `GameState Ended` before closing it (#462). |
