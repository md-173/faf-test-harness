...
- **The `faf-uid` binary**, referenced by the `uidBinaryPath` config key. This
  ...
  rejects a placeholder `unique_id`, and without it the login ends in
  `{"command":"invalid"}`, which `run` reports at once as `the lobby answered
  the login with invalid`. There is no way around it for a real
...
`{"command":"invalid"}` rather than an auth error. `run` names it in its one
ERROR, `the lobby answered the login with invalid`, and exits `70` at once.
Keep `--uid-binary-path` pointed at a real `faf-uid`.
...
`{"command":"invalid"}` and a closed connection, which `run` reports as `the
lobby answered the login with invalid`. That means either the lobby
...
| How | Exit code | Notes |
|---|---|---|
| `Ctrl-C` / `SIGTERM` | 130 (SIGINT) / 143 (SIGTERM) on Linux/macOS | Clean: closes the WebSocket via a JVM shutdown hook, having first sent `GameState Ended` for a game it had launched or was still launching, logs `shutdown signal received; tearing down session`, no error lines, and no verdict line, since the code reported is the signal's; the exception is the instant where the session's own end and the signal land together, when one can still be logged. That line marks a signal and nothing else: a run that ends on its own never logs it, whatever its code. A Ctrl-C during bring-up can also log the launch's cause at WARN, because the terminal's SIGINT reaches the adapter as well. On native Windows, POSIX signal numbers don't apply: expect a platform-specific code rather than literally 130/143 (see the same caveat in §2). These are the launcher's own codes, observable directly only when it is invoked as in §5; running it through `./gradlew :mock-client:run` instead reports Gradle's own `1` for every non-zero case regardless of the underlying code. |
...
| Runtime failure after start (bad refresh-token file, lobby session failure), or a game launch whose ICE adapter or game never came up | `70` (`RUNTIME`) | A failed launch logs its cause first (`Could not read the game_launch frame`, `Could not launch the ICE adapter`, `Could not connect or setup the ICE adapter` or `Could not launch game binary`), then `the ICE adapter or game never came up; reporting it in this run's exit code`. A `game_launch` the client cannot use (a required field missing, a value of the wrong JSON type, which it refuses rather than converts, such as `"uid":1.5` or `"mod":5`, or one the launch checks refuse, such as an unknown `args` flag) ends the run this way as it arrives, and its cause line names the field; the run used to wait, until killed, for a launch it had dropped. `run` does not check the binaries before it logs in, so a wrong path only surfaces once the lobby sends `game_launch`. An unexpected exception on the way up is a bug, not a finding about the adapter: its cause line is `Could not launch the game session` at ERROR with the stack trace, and the run ends the same way instead of waiting forever. Once a launch has started, `run` still sends the lobby `GameState Ended` before closing it, like the real client (#462); a refused `game_launch` starts none. A lobby that ends the login before `welcome` ends the run this way at once, its one ERROR naming how (§7), where it used to wait out a 45 s setup timeout. So does a lobby connection that drops under a live session, logged as `lobby connection dropped unexpectedly`, a drop without a Close frame (code 1006) included. |
| A session that failed after its adapter and game came up | `70` (`RUNTIME`) | A `HostGame`, `JoinGame` or `ConnectToPeer` frame the client could not read, a `DisconnectFromPeer` one before the game started, an adapter that answered a host, join or peer-connect call with an error or not within 5 s, an adapter still running once the session ended although its JSON-RPC link had closed from its side, such as one whose stream stopped parsing, or, when matchmaking, a match the server cancelled after `game_launch` and before the game started (once `Match started` is logged, the match plays on, #304). The cause is logged first (`Could not host the game`, `Could not join the game`, `Could not connect to a peer`, `Could not disconnect from a peer`, `Could not set up the peer relay for id=<id>`, `ICE adapter JSON-RPC link dropped while the adapter kept running` or `match_cancelled after game_launch`), then `the session failed after its ICE adapter and game came up; reporting it in this run's exit code`. An unexpected exception in the host, join or peer-connect step is logged like the launch's, at ERROR with its trace. A call that failed because the adapter's connection closed is not this by itself, so the line naming the call is INFO: teardown decides it once the session ends, as the `72` row when the adapter died, and as this row when the adapter is still running without its link. Checked after `72` and `71`, so a run whose adapter or game died reports that instead. |
| The game process died unaccounted for | `71` (`GAME_CRASHED`) | A non-zero game exit with no `GameEnded` observed and no teardown of ours, logged as `mock-game exited abnormally`. mock-game's own `69` is excluded and logged as a lost adapter link instead; it never produces a code by itself. The adapter's own death is the row below, keyed on the adapter's exit. |
| The ICE adapter died mid-session | `72` (`ADAPTER_LOST`) | A non-zero adapter exit while the session was live, outside teardown, logged as `ICE adapter exited abnormally`. An adapter that was up and then died; a launch that never came up is `70`, above. Checked ahead of `71`, so a session that lost its adapter reports the adapter rather than whatever its game did in response, and after `70`: a run whose lobby connection also dropped reports `70`. Also when something the death caused ends the session first, an adapter RPC call failing in flight or the game's own exit processed first: teardown finds the adapter dead before it stops it, waiting up to two seconds for one whose link has just closed, and reports this. |
...
|---|---|---|
| `run` exits `70` at once, without a `session ready` line | `lobby session with <url> failed: AuthenticationException: the lobby answered the login with invalid, …`, the lobby's `{"command":"invalid"}` | The lobby's policy server rejected a placeholder `unique_id`: `uidBinaryPath` is unset or wrong. Set it to a real `faf-uid` binary (§1, §3). There is no way to reach a live session without this. Less often, the lobby could not read the token (§3). |
| `run` exits `70` at once, without a `session ready` line | `lobby session with <url> failed: AuthenticationException: the lobby closed the connection before welcome (code 1000)`, usually after a WARN `lobby notice (error): <text>` | The lobby refused the login and closed the connection, and its notice says why, such as a ban or a lobby database outage. With no notice, it could not parse the `auth` frame. `the lobby connection dropped before welcome` means the connection was lost rather than closed. |
...
These flags degrade the harness from the inside, covering all three faults the
project brief names: delayed ICE negotiation, dropped UDP packets, and game
crash. Each defaults to off, and with them unset nothing about a run changes.
Validation is by log inspection, which is what the brief itself asks for.

None of them needs elevated privileges, a specific operating system, or any
network configuration, so all of them run unchanged inside a container.
...
- Exit code `134` (`128 + SIGABRT`), which is what a POSIX shell reports for an
  ...
  `69`, `70`, `75`) and from the JVM's own `1` and `143`, so an injected crash is
  never confused with a genuine adapter loss.

In an orchestrated run, the surviving client then logs `mock-game exited
abnormally with exit code 134`, reaches TERMINATED, sends the synthesised
`GameState Ended` to the lobby on the game's behalf as it tears down, and exits
`71` (`GAME_CRASHED`) rather than `0`.
